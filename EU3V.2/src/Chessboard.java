...
public class Chessboard {
  ...
  public boolean isValidField(char row, int column) {
	if (row >= 'a' && row <= 'h' && column > 0 && column <= NUMBER_OF_COLUMNS) {
	  return true;
	} else {
	  return false;
	}
  ...
  public class Rook extends Chesspiece {
	...
	public void markReachableFields() {
	  ...
	  for (byte col = 1; col <= NUMBER_OF_ROWS; col++) {
		if (Chessboard.this.isValidField(row, col)) {
		  int r = row - FIRST_ROW;
		  int c = col - FIRST_COLUMN;
		  Chessboard.this.fields[r][c].mark();
	...
	public void markReachableFields() {
	  ...
	  for (int i = 0; i < NUMBER_OF_COLUMNS; i++) {
		if (Chessboard.this.isValidField((char) (row + xr[i]), (column + xc[i]))) {
		  int r = (char) (row + xr[i] - FIRST_ROW);
		  int c = (byte) (column + xc[i] - FIRST_COLUMN);
		  Chessboard.this.fields[r][c].mark();
		}
	  }
	}
